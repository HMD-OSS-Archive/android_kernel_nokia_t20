		...
		struct device_attribute *attr, char *buf)
{
	...
	brightness = bd->props.brightness*100/95;
	if(brightness >=254)
		brightness = 255;
...
static ssize_t boost_brightness_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct backlight_device *bd = to_backlight_device(dev);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rc)
		return rc;
	if(boost_mode == 0)
		brightness = brightness*95/100;
	else if(brightness < 229)
		brightness = brightness*95/100;
...
static ssize_t boost_brightness_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int rc;
	struct backlight_device *bd = to_backlight_device(dev);
...
static DEVICE_ATTR_RW(boost_brightness);

static ssize_t boost_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int rc;
	rc = kstrtoint(buf, 0, &boost_mode);
	return count;
}
...
static struct attribute *bl_device_attrs[] = {
	...
	&dev_attr_boost_brightness.attr,
