...
struct sprd_asoc_hook_spk_priv {
    ...
    int state[BOARD_FUNC_MAX];  // -1:virtual on; 0: off; >0:real on(mode)
...
/* rcv shared the ext pa path of spk */
static int hook_general_ctl(int id, int on)
{
    ...
    case BOARD_FUNC_SPK:
        ...
        if (on && 0 < hook_spk_priv.state[BOARD_FUNC_EAR]) {
            hook_general_spk(BOARD_FUNC_EAR, 0);
            if (hook_spk_priv.rcv_shared_multi_spk) {
                select_mode = 0;
                hook_general_spk(BOARD_FUNC_SPK1, 0);
            }
            hook_spk_priv.state[BOARD_FUNC_EAR] = -1;			// ear virtual on
        }

        // spk on/off
        hook_general_spk(id, on);
        if (hook_spk_priv.rcv_shared_multi_spk) {
            select_mode = 0;
            hook_general_spk(BOARD_FUNC_SPK1, on);
        }

        // when spk off, if ear is virtual on: ear switch to real on
        if (!on && -1 == hook_spk_priv.state[BOARD_FUNC_EAR]) {
            hook_general_spk(BOARD_FUNC_EAR, 1);				// ear real on
            if (hook_spk_priv.rcv_shared_multi_spk) {
                select_mode = hook_spk_priv.priv_data[BOARD_FUNC_EAR];
                hook_general_spk(BOARD_FUNC_SPK1, 1);
                select_mode = 0;
            }
        }
        break;
    ...
    case BOARD_FUNC_EAR:
        ...
        if (0 < hook_spk_priv.state[BOARD_FUNC_SPK]) {
            ...
            hook_general_spk(BOARD_FUNC_EAR, on);				// ear real on/off
            if (hook_spk_priv.rcv_shared_multi_spk) {
    ...
    pr_info("%s hook state {%d, %d, %d}", __func__,
...
static struct sprd_asoc_ext_hook_map ext_hook_arr[] = {
	...
	{"rcv_through_ext_spk", hook_general_ctl, EN_LEVEL},
};
					 ...
					 struct sprd_asoc_ext_hook *ext_hook)
{
	...
	/* Parse configs for whether rcv shared with multi spk. */
	hook_spk_priv.rcv_shared_multi_spk = of_property_read_bool(np, "sprd,rcv_shared_multi_spk");
	dev_info(dev, "[%s][20210525] rcv shared with multi spk: %d\n",
