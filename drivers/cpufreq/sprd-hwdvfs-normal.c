			     ...
			     const struct i2c_device_id *id)
{
	...
	if (dcdc >= pdev->dcdc_num) {
		dev_err(pdev->dev, "the dcdc id that used i2c channel is overflowing\n");
		ret = -EINVAL;
		goto child_node_put;
	}
...
{
	u32 new_data = readl((pdev->membase + reg)) & ~msk;

	new_data |= val & msk;

	writel(new_data, (pdev->membase + reg));
}
			     ...
			     int count)
{
	...
	if (index < 0 || index > count) {
		pr_err("%s: incorrcet voltage gear table index, index: %d, count = %d\n",
	...
	pr_debug("TOP_DVFS_VOL_GRADE_TBL[%d]: reg = 0x%x, off = %d, msk = 0x%x, val = 0x%x\n",
...
static void find_maximum_vol_diff(unsigned long *vol, u32 *max_diff_val,
				  int vol_size, int n)
{
	int i, j;
...
static int sprd_dvfs_module_eb(struct cpudvfs_device *pdev)
{
	...
	pdev->aonmap = syscon_regmap_lookup_by_phandle_args(node,
				"enable-syscon", 2, args);
	if (IS_ERR(pdev->aonmap)) {
		dev_err(pdev->dev, "no dvfs module enable-syscon\n");
...
int sprd_dvfs_block_dcdc_shutdown_enable(struct cpudvfs_device *pdev, u32 num)
{
	...
	blk_sd_map = syscon_regmap_lookup_by_phandle_args(dcdc_node,
				"dvfs-blk-sd-syscon", 2, syscon_args);
	if (IS_ERR(blk_sd_map)) {
		dev_err(pdev->dev, "no regmap for dvfs block shutdown syscon\n");
...
int sprd_mpll_relock_enable(struct cpudvfs_device *pdev, u32 num)
{
	...
	if (!pdev->mplls[num].output_switch_done) {
		dev_err(pdev->dev, "the output of mpll%d is not be switched to hardware\n",
			num);
		return -EINVAL;
	}
...
int sprd_mpll_pd_enable(struct cpudvfs_device *pdev, u32 num)
{
	...
	if (!pdev->mplls[num].output_switch_done) {
		dev_err(pdev->dev, "the output of mpll%d has not be switched to hardware",
			num);
		return -EINVAL;
	}
...
int sprd_cpudvfs_cluster_dvfs_enable(void *data, u32 clu_id, bool enable)
{
	...
	while (slave) {
		if (slave->dcdc == clu->dcdc) {
			ret = sprd_slave_cluster_auto_tune_enable(pdev,
								  slave->id,
								  enable);
			if (ret)
				return ret;
	...
	return sprd_host_cluster_dvfs_enable(pdev, clu_id, enable);
}
...
static int sprd_mpll_table_init(struct cpudvfs_device *pdev, u32 mpll_num)
{
	...
	while (!(!regdata->reg && !regdata->off && !regdata->msk &&
		 !regdata->val)){
		dev_dbg(pdev->dev, "MPLL%d-index%d-icp: reg = 0x%x, msk = 0x%x, off = %d, val = 0x%x\n",
		...
		dev_dbg(pdev->dev, "MPLL%d-index%d-postdiv: reg = 0x%x, msk = 0x%x, off = %d, val = 0x%x\n",
			mpll_num, i, regdata->reg, regdata->msk, regdata->off,
			regdata->val);
		...
		dev_dbg(pdev->dev, "MPLL%d-index%d-n: reg = 0x%x, msk = 0x%x, off = %d, val = 0x%x\n",
			mpll_num, i, regdata->reg, regdata->msk, regdata->off,
			regdata->val);
...
static int sprd_hw_dvfs_misc_config(struct cpudvfs_device *pdev)
{
	...
	while (!(!en->reg && !en->off && !en->msk && !en->val)) {
		dev_dbg(pdev->dev, "TOP_DVFS_MISC_CFG[%d]: reg = 0x%x, msk = 0x%x, off = %d, val = 0x%x\n",
	...
	while (!(!en->reg && !en->off && !en->msk && !en->val)) {
		dev_dbg(pdev->dev, "APCPU_DVFS_MISC_CFG[%d]: reg = 0x%x, msk = 0x%x, off = %d, val = 0x%x\n",
			j, en->reg, en->msk, en->off, en->val);
...
static int sprd_setup_i2c_channel(struct cpudvfs_device *pdev, u32 dcdc_nr)
{
	...
	if (pdev->pwr[dcdc_nr].i2c_used && pdev->pwr[dcdc_nr].i2c_shared) {
		...
		if (ret)
			...
			dev_info(pdev->dev, "the dcdc%d shares the i2c channel with other devices\n",
				 dcdc_nr);
...
static int sprd_dvfs_device_init(struct cpudvfs_device *pdev)
{
	...
	for (ix = 0; ix < pdev->dcdc_num; ++ix) {
		...
		if (ret) {
			dev_err(pdev->dev, "failed to enable dvfs to block dcdc%d to shutdown\n",
				ix);
			return ret;
		}
					   ...
					   unsigned long volt, int opp_idx)
{
	...
	if (grade_id >= MAX_VOLT_GRADE_NUM) {
		dev_err(pdev->dev, "the volt grade number(%d) is beyond the maximun(%d)\n",
	...
	if (ret) {
		dev_err(pdev->dev, "error in updating volt gear values for dcdc%d\n",
			clu->dcdc);
		return ret;
	}
...
init_map:
	...
	while (slave) {
		if (slave->dcdc == clu->dcdc) {
			...
			dev_dbg(pdev->dev,
				"update dvfs index map tbl for slave cluster%d (%s)that belong to the host cluster%d\n",
				idx, slave->dts_tbl_name, clu->id);
...
int sprd_cpudvfs_idle_pd_volt_update(void *data, u32 clu_id)
{
	...
	if (ret) {
		dev_err(pdev->dev, "failed to set idle voltage for dcdc-cpu%d\n",
			clu->dcdc);
		return ret;
	}
 ...
 */
static struct cpudvfs_phy_ops sprd_cpudvfs_phy_ops = {
...
static  int sprd_mpll_dt_parse(struct cpudvfs_device *pdev)
{
	...
	num = num / sizeof(u32);
	if (num > MAX_MPLL) {
		dev_err(pdev->dev, "the number of mpll(%d) is beyond the maximum value(%d)",
			num, MAX_MPLL);
		return -EINVAL;
	}
	...
	for (idx = 0; idx < num; ++idx) {
		...
		map = syscon_regmap_lookup_by_phandle_args(node,
					"mpll-sel-syscon", 2, args);
		if (IS_ERR(map)) {
			dev_err(pdev->dev, "no mpll-sel-syscon found\n");
			ret = PTR_ERR(map);
			goto mpll_free;
		} else {
...
static int sprd_dcdc_dt_parse(struct cpudvfs_device *pdev)
{
	...
	map = syscon_regmap_lookup_by_phandle(node, "sprd,topdvfs-sysctrl");
	if (IS_ERR(map)) {
		dev_err(pdev->dev, "no regmap for syscon topdvfs-sysctrl\n");
		ret = -ENODEV;
		goto err_out;
	}
...
static int sprd_cluster_detail_parse(struct dvfs_cluster *clu)
{
	...
	if (!of_find_property(node, clu->dts_tbl_name, NULL)) {
		dev_err(pdev->dev, "no %s property found\n", clu->dts_tbl_name);
		ret = -EINVAL;
		goto child_node_put;
	}
...
static int sprd_cluster_dt_parse(struct cpudvfs_device *pdev)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (dt_clu_num != clu_num) {
			dev_err(pdev->dev, "the number of %s defined in dts is incorrect\n",
				name);
			ret = -EINVAL;
			goto err_opp_free;
		}
...
static int get_hw_version_id_by_cmdline(void)
{
	...
	if (!rc) {
		p = strstr(cmd_line, "hw.version=");
        if (p == NULL) {
                printk("hw.version= is not exit\n");
                return 0;
        } else {
                p += 11;
                strncpy(&ret, p, 1);
		...
		printk("can't not parse bootargs hw.version property\n");
		return 1;
	}
}
#endif
...
#ifdef CONFIG_GET_HW_VERSION
	if(get_hw_version_id_by_cmdline() <= 0) {
		printk("hw v1.0 exit cpufreq probe\n");
		return -1;
	}
#endif
