...
static int sprd_panel_unprepare(struct drm_panel *p)
{
    ...
    if (panel->info.tp3v3en_gpio) {
            gpiod_direction_output(panel->info.tp3v3en_gpio, 0);
            mdelay(5);
    }
...
static int sprd_panel_prepare(struct drm_panel *p)
{
    ...
    if (panel->info.tp3v3en_gpio) {
            gpiod_direction_output(panel->info.tp3v3en_gpio, 1);
            mdelay(5);
    }
			...
			struct sprd_panel *panel)
{
    ...
    panel->info.tp3v3en_gpio = devm_gpiod_get_optional(dev,
                                    "tp3v3en", GPIOD_ASIS);
    if (IS_ERR_OR_NULL(panel->info.tp3v3en_gpio))
            DRM_WARN("can't get panel tp3v3en gpio: %ld\n",
                             PTR_ERR(panel->info.tp3v3en_gpio));
