...
static void sc27xx_cc_polarity_status(u32 status)
{
	...
	if (status & SC27XX_TYPEC_FINAL_SWITCH)
		cc_status = SPRD_TYPEC_POLARITY_CC1;
	else
		cc_status = SPRD_TYPEC_POLARITY_CC2;
}
...
static struct kobj_attribute cc_status_attribute =  __ATTR(cc_status, 0644, status_show, status_store);
static struct kobject *cc_typec_kobj;

static int cc_status_init(void)
{

   int retval;

   cc_typec_kobj = kobject_create_and_add("cc_typec", NULL);

   if (!cc_typec_kobj)
             return -ENOMEM;

   retval = sysfs_create_file(cc_typec_kobj, &cc_status_attribute.attr);

   if (retval)

      kobject_put(cc_typec_kobj);
