...
struct charger_manager *em_cm;
EXPORT_SYMBOL_GPL(em_cm);
...
static int cm_get_target_status(struct charger_manager *cm)
{
	...
	if(em_temp_up_down == true && cm->desc->temperature >= 430 && vbat_avg >= 4200000){
		printk("sqw :43~54 vbat > 4200000 temp is up,stop it\n ");
		return POWER_SUPPLY_STATUS_NOT_CHARGING;
	}
